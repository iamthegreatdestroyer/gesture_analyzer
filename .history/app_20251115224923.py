...
class MainWindow(QMainWindow):
    ...
    def select_video(self) -> None:
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Select Video", str(Path.cwd()), "Video Files (*.mp4 *.avi *.mov *.mkv)"
        )
        if file_path:
    ...
    def run_analysis(self) -> None:
        if not self.selected_video or not self.selected_video.exists():
            QMessageBox.warning(self, "No Video", "Please select a valid video first.")
            return
        try:
            ...
            row_id = db.save_analysis(str(self.selected_video), result.dataframe, result.summary)

            pretty = json.dumps(result.summary, indent=2)
            self.output.setPlainText(f"Analysis saved (id={row_id}):\n\n{pretty}")
            self.table.setVisible(False)
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Failed to analyze video:\n{e}")
        ...
        try:
            ...
            for r in range(len(catalog)):
                for c, col in enumerate(catalog.columns):
                    self.table.setItem(r, c, QTableWidgetItem(str(catalog.iloc[r][col])))
