...
def distance(a: Tuple[float, float, float], b: Tuple[float, float, float]) -> float:
    return float(np.linalg.norm(np.array([a[0]-b[0], a[1]-b[1], a[2]-b[2]])))


def is_ok_sign(coords: Dict[int, Tuple[float, float, float]]) -> bool:
    """OK sign: thumb tip touches index fingertip; other three fingers extended."""
...
def is_point(coords: Dict[int, Tuple[float, float, float]]) -> bool:
    """Point: index finger extended; other fingers folded; thumb across palm (tip near index MCP)."""
