...
def process_video(
    ...
    if mp_hands is None:
        ...
        logger.warning(
            "MediaPipe unavailable; returning empty result for %s", video_path
        )
        ...
        while True:
            ...
            if result.multi_hand_landmarks:
                ...
                for h_idx, hand_landmarks in enumerate(
                    ...
                    # Per-frame gesture classification with confidence scores
                        ...
                        gesture_events.append(
                            {
                                ...
                                "hand_index": h_idx,
                                "label": "thumbs_up",
                                "confidence": thumbs_up_confidence(
                                    coords, label
                                ),
                            }
                        )
                        ...
                        gesture_events.append(
                            {
                                ...
                                "hand_index": h_idx,
                                "label": "ok",
                                "confidence": ok_confidence(coords),
                            }
                        )
                    if is_pinch(coords):
                        gesture_events.append(
                            {
                                ...
                                "hand_index": h_idx,
                                "label": "pinch",
                                "confidence": pinch_confidence(coords),
                            }
                        )
                    if is_point(coords):
                        gesture_events.append(
                            {
                                ...
                                "hand_index": h_idx,
                                "label": "point",
                                "confidence": point_confidence(coords),
                            }
                        )
                            ...
                            gesture_events.append(
                                {
                                    ...
                                    "label": "wave",
                                    "confidence": min(
                                        1.0, wrist_toggle_count[h_idx] / 8.0
                                    ),
                                }
                            )
    ...
    logger.info(
        "Completed processing: %s frames=%s gestures=%s",  # noqa: E501
        video_path,
        summary.get("frames_total"),
...
def thumbs_up_confidence(
    ...
    wrist_y = coords[0][1]
    thumb_tip_y = coords[4][1]
    lift = clamp01((wrist_y - thumb_tip_y - 0.02) / 0.08)
    ang = 0.0
    if all(k in coords for k in [1, 2, 3]):
        ang_raw = compute_angle(coords[1], coords[2], coords[3])
        ang = clamp01((ang_raw - 120) / 60)
    side_score = 0.5
    index_mcp_x = coords[5][0]
    thumb_tip_x = coords[4][0]
    hl = handedness.lower()
    if hl == "right":
        side_score = clamp01((index_mcp_x - thumb_tip_x - 0.005) / 0.05)
    elif hl == "left":
        side_score = clamp01((thumb_tip_x - index_mcp_x - 0.005) / 0.05)
    return clamp01(0.4 * lift + 0.4 * ang + 0.2 * side_score)


def ok_confidence(coords: Dict[int, Tuple[float, float, float]]) -> float:
    if not all(k in coords for k in [4, 8]):
        return 0.0
    dist = distance(coords[4], coords[8])
    return clamp01((0.07 - dist) / 0.07)


def pinch_confidence(coords: Dict[int, Tuple[float, float, float]]) -> float:
    if not all(k in coords for k in [4, 8]):
        return 0.0
    dist = distance(coords[4], coords[8])
    return clamp01((0.05 - dist) / 0.05)


def point_confidence(coords: Dict[int, Tuple[float, float, float]]) -> float:
    if not all(k in coords for k in [8, 6]):
        return 0.0
    # Extension amount
    ext = clamp01((coords[6][1] - coords[8][1] - 0.005) / 0.05)
    return ext
