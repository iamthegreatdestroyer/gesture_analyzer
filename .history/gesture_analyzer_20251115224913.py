...
def process_video(video_path: str, max_frames: int | None = None) -> ProcessResult:
    """
    ...
    handedness_rows: Dict[int, List[str]] = {}
        ...
        while True:
            ...
            if result.multi_hand_landmarks:
                ...
                if result.multi_handedness:
                    labels = [h.classification[0].label for h in result.multi_handedness]

                for h_idx, hand_landmarks in enumerate(result.multi_hand_landmarks):
    ...
    df = pd.DataFrame(
        rows, columns=["frame", "hand_index", "handedness", "landmark_id", "x", "y", "z"]
    )
...
def analyze_patterns(df: pd.DataFrame) -> Dict:
    ...
    - approx_repetitions_per_hand: estimate repetitions from wrist (lm 0) x/y zero-crossings
    ...
    for hand_idx, g in df[df["landmark_id"] == 0].groupby("hand_index"):
        ...
        thresh = max(1e-4, 0.2 * np.std(dx + 1e-9))
