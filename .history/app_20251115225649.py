...
class MainWindow(QMainWindow):
    ...
    def apply_filter(self) -> None:
        ...
        if text:
            mask = (
                df["video_name"].astype(str).str.contains(text, case=False, na=False)
                | df["analysis_date"].astype(str).str.contains(text, case=False, na=False)
                | df["patterns_json"].astype(str).str.contains(text, case=False, na=False)
            )
    ...
    def export_csv(self) -> None:
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Save CSV", str(Path.cwd() / "catalog.csv"), "CSV Files (*.csv)"
        )
        if path:
    ...
    def export_json(self) -> None:
        if self._filtered_df is None or self._filtered_df.empty:
            ...
            return
        path, _ = QFileDialog.getSaveFileName(
            self, "Save JSON", str(Path.cwd() / "catalog.json"), "JSON Files (*.json)"
        )
        if path:
