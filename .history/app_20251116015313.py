...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        logging.getLogger(__name__).info("Initializing main window")
        db.init_db()
        ...
        # Make path label selectable if flag exists (PyQt5 versions differ)
        if hasattr(Qt, "TextSelectableByMouse"):
            try:
                self.path_label.setTextInteractionFlags(
                    getattr(Qt, "TextSelectableByMouse")
                )
            except Exception:
                pass
