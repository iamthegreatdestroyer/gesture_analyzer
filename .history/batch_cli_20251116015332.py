...
Usage:
    python batch_cli.py --input-dir videos/ --pattern *.mp4 \
        --output results.json --format json
    python batch_cli.py --input-dir videos/ --save-db --format csv \
        --output summary.csv
...
Options:
    ...
    --output FILE       Output path (json or csv). If omitted prints summary.
...
def collect_videos(input_dir: str, pattern: str) -> List[Path]:
    pattern_path = str(Path(input_dir) / pattern)
    paths = [Path(p) for p in glob.glob(pattern_path, recursive=False)]
...
def main() -> None:
    ...
    # args.input_dir is canonical; fallback kept for defensive coding if hyphenated
    input_dir = getattr(args, "input_dir", getattr(args, "input-dir", None))
    videos = collect_videos(input_dir, args.pattern)
    if not videos:
