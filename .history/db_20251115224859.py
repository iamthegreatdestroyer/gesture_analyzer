...
DB_PATH = Path(os.environ.get("GESTURE_DB_PATH", "data/analytics.db")).resolve()
...
def save_analysis(video_path: str, df: pd.DataFrame, summary: Dict) -> int:
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        cur.execute(
            """
            INSERT INTO video_analytics (video_name, analysis_date, patterns_json, raw_data_csv)
            VALUES (?, ?, ?, ?)
            """,
            (
                video_name,
...
def query_catalog(limit: Optional[int] = None) -> pd.DataFrame:
    ...
    _ensure_dirs()
    with sqlite3.connect(DB_PATH) as conn:
        df = pd.read_sql_query("SELECT * FROM video_analytics ORDER BY id DESC", conn)
